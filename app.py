...
st.markdown("""
    ...
    /* Imagens padronizadas e niveladas */
    ...
    /* Títulos e Descrições nivelados */
    .jogo-titulo {
        height: 50px;
        margin-top: 10px;
        font-size: 0.95rem;
...
def main():
    ...
    with st.sidebar:
        ...
        cats = sorted(list(set([g['type'] for g in dados])))
        f_cat = st.multiselect("🏷️ Tipo de Brinde:", cats, placeholder="Selecione os tipos")
        
        min_v = st.slider("💵 Valor original min. ($):", 0, 100, 0)
        
        plats = sorted(list(set([traduzir_plataforma(g['platforms']) for g in dados])))
        f_plat = st.multiselect("🖥️ Plataformas:", plats, placeholder="Selecione as plataformas")
    ...
    for i in range(0, len(jogos), cols_count):
        ...
        for j, jogo in enumerate(linha):
            with cols[j]:
                ...
                st.markdown(f'<div class="jogo-desc">{jogo["description"]}</div>', unsafe_allow_html=True)
                ...
                d_fim = jogo.get('end_date', 'N/A')
                txt_d = f"⏰ Expira em: {d_fim}" if d_fim != "N/A" else "⏰ Tempo limitado!"
                st.markdown(f'<div style="font-size:0.75rem; color:#777;">📍 {traduzir_plataforma(jogo["platforms"])}</div>', unsafe_allow_html=True)
                st.markdown(f'<div style="font-size:0.75rem; color:#d32f2f; font-weight:bold; margin-bottom:10px;">{txt_d}</div>', unsafe_allow_html=True)
